...
per-child fill modifier is what produces the stretch semantics.

Note: Compose's semantics-tree snapshot doesn't capture layout modifiers
like `fillMaxWidth`, only structural and accessibility data. The fix
changes runtime layout (the visible behavior) but the committed
`snapshot.android.txt` files are unchanged. A snapshot regression of the
stretch behavior would not be caught by the current snapshot tests; pixel
screenshot tests, if added in Phase 4, would close that gap.
...
- Robolectric was not in the Phase 2 plan's enumerated deps. We adopt it as
  ...
  thorough `check`.
- `alignment: stretch` is implemented by per-child fill modifiers and
  achieves CSS-equivalent semantics, but isn't reflected in the
  semantics-tree snapshot. Visual regressions of the stretch behavior
  would not be caught by `./gradlew check`; addressed when (or if) we
  add pixel screenshot tests.
